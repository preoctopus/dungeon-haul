...
describe("isC2SNameEntry validation (codec.ts lines 119-129)", () => {
  it("accepts a valid name entry", () => {
    ...
    expect(
      isC2SNameEntry({ type: "name_entry", name: "Player_1" }),
    ).toBe(true);
    expect(isC2SNameEntry({ type: "name_entry", name: "Bob.Bob's" })).toBe(
      true,
    );
  });
  ...
  it("rejects names with disallowed characters", () => {
    expect(isC2SNameEntry({ type: "name_entry", name: "bad@char" })).toBe(
      false,
    );
...
describe("WorldSnapshot contract (protocol core.ts)", () => {
  it("constructs a valid WorldSnapshot for lobby phase", () => {
    ...
    expect(snapshot.treasures).toEqual([]);
  });
  ...
  it("round-trips through JSON without losing structure", () => {
    const original = {
      ...
      levelsCompleted: 3,
      levelsAfterHoard: 5,
      lastProcessedInputSeq: [10, 8, 6, 4],
      haulers: [],
      treasures: [],
      traps: [],
      switches: [],
  ...
  it("uses seatId array type for lastProcessedInputSeq", () => {
    // The protocol uses an array [0, 1, 2, 3] not a record object.
    const snapshot = {
      tick: 0,
      phase: "lobby" as const,
      levelsCompleted: 0,
      levelsAfterHoard: 2,
      lastProcessedInputSeq: [0, 1, 2, 3],
      haulers: [],
      treasures: [],
      traps: [],
      switches: [],
    };
    expect(Array.isArray(snapshot.lastProcessedInputSeq)).toBe(true);
    expect(snapshot.lastProcessedInputSeq.length).toBe(4);
  });
});
...
describe("ScoreReport contract (protocol score.ts)", () => {
  it("constructs a valid ScoreReport", () => {
    ...
    expect(report.perSeat.length).toBe(1);
...
/** Helper that parses the encodeMessage output for assertions. */
...
describe("encodeMessage round-trip", () => {
  ...
  it("encodes and decodes an end_shares message with perSeat data", () => {
    const msg = encodeMessage({
      ...
      sessionId: "sess-end",
      perSeat: [
        { seatId: 0, gp: 20, shares: 4, rank: 1 },
